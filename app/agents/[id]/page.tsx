...
export default function AgentDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      const found = agents.find((a: any) => a.id.toString() === params.id);
  ...
  const activities = useMemo<Activity[]>(() => {
    ...
    for (const token of marketData) {
      ...
      const premium = token.premium ?? ((Math.random() - 0.5) * 40);
      ...
      if (discount > threshold + 5 && Math.random() > 0.5) {
        log.push({
          type: "bought",
          symbol: token.symbol,
          message: `Bought ${(Math.random() * 10 + 1).toFixed(2)} ${token.symbol} at $${token.markPrice.toFixed(2)}`,
        ...
        log.push({
          ...
          detail: `${premium.toFixed(1)}% premium, flagged as overvalued — skipped`,
          time: "Yesterday",
        });
      } else {
        log.push({
          ...
          detail: `Insufficient discount (${discount.toFixed(1)}%) — minimum required: ${threshold}%`,
  ...
  const handleKill = () => {
    if (!confirm("Stop agent? This will liquidate the vault and return funds to Main Vault.")) return;
    const stored = localStorage.getItem("matka_agents");
    if (stored) {
      const agents = JSON.parse(stored);
      localStorage.setItem("matka_agents", JSON.stringify(agents.filter((a: any) => a.id.toString() !== params.id)));
    }
    alert("Kill switch triggered — vault liquidated!");
  ...
  if (!agent) return (
    <div className="p-8">
      ...
      <Link href="/home" className="text-blue-400 mt-2 inline-block">← Back</Link>
    </div>
  );
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <Link href="/home" className="text-gray-500 text-sm hover:text-white">← Dashboard</Link>
        <button
          onClick={handleKill}
          className="bg-red-900/20 text-red-500 border border-red-800 hover:bg-red-800 hover:text-white px-4 py-2 rounded-lg font-bold text-sm transition-colors"
        >
          🛑 KILL SWITCH
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        <div className="col-span-1 space-y-5">
          <div className="bg-gray-900 border border-gray-800 p-5 rounded-xl">
            <div className="text-blue-400 text-xs font-bold uppercase tracking-widest mb-2">{agent.category}</div>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                <div className="text-gray-500 text-xs uppercase">Current Value</div>
                <div className="font-bold">${currentValue.toFixed(2)}</div>
              </div>
              <div>
                <div className="text-gray-500 text-xs uppercase">P&L</div>
                <div className="font-bold text-green-400">+${pnl.toFixed(2)} (+{pnlPct.toFixed(1)}%)</div>
              </div>
              <div>
          ...
          <div className="bg-gray-900 border border-gray-800 p-5 rounded-xl">
            <h3 className="font-bold text-sm mb-3 text-gray-300">On-Chain Rules</h3>
            <div className="space-y-2 text-xs text-gray-400">
              <div className="flex justify-between"><span>Discount threshold</span><span className="text-white">&gt;{agent.rules.discount}%</span></div>
              <div className="flex justify-between"><span>Max per trade</span><span className="text-white">{agent.rules.max_trade}%</span></div>
              <div className="flex justify-between"><span>Pre-IPO cap</span><span className="text-white">{agent.rules.max_preipo}%</span></div>
              <div className="flex justify-between"><span>Watching</span><span className="text-white">{marketData.length} tokens</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-3">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-500">Auto-updating from ClawPump agent</span>
          </div>

          <div className="space-y-3">
            {activities.map((activity, i) => {
              const iconMap = { bought: "🟢", checked: "🔵", skipped: "🟡", declined: "🔴" };
              return (
                <div
                  key={i}
                  className="bg-gray-900 border border-gray-800 hover:border-gray-700 p-4 rounded-xl flex gap-4 transition-colors"
                >
                  <div className="text-xl flex-shrink-0">{iconMap[activity.type]}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-400 text-xs mt-0.5">{activity.detail}</p>
                    {activity.txHash && (
                      <a
